...
function isSolarTerm (year,month){
  let obj = {}
  let day1 = sTerm(year, month * 2);
  let day2 = sTerm(year, month * 2 + 1);
  obj[day1] = solarTerm[month * 2];
  obj[day2] = solarTerm[month * 2 + 1];
  return obj 
}
...
function calendar(y, m) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    if (lD > lX) {
      lD = day;
      lX = isLeap ? leapDays(year) : monthDays(year, month);
  ...
  // 节气数组 {date: 节气名} date 需要-1
  let Solar = isSolarTerm(y,m);
  for(let j in Solar){
    dateData[j-1].solarTerms = Solar[j]
  }
...
F = 365.242 * (2020 - 1900) + 6.2 + 15.22 *4 - 1.9 * Math.sin(0.262 * 4)
